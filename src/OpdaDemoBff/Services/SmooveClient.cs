...
public sealed class SmooveClient : ISmooveClient, IDisposable
{
    ...
    public static async Task<SmooveClient> CreateAsync(string baseUrl, string apiKeyPath)
    {
        ...
        return new SmooveClient(http);
    }

    private SmooveClient(HttpClient http) => _http = http;
